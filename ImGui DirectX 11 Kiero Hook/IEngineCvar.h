...
public:
	char pad01[0x40];
	// Instead of void*, let's use a structure to represent the linked list
	struct ConVarNode {
		CConVar* pConVar;
		ConVarNode* next;
	};
	ConVarNode* convarList;

	void UnlockHiddenCVars()
	{
		ConVarNode* currentNode = convarList;
		while (currentNode != nullptr)
		{
			CConVar* pConVar = currentNode->pConVar;
			if (pConVar != nullptr) {
